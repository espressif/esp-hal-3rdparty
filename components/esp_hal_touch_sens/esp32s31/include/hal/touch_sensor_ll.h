...
{
    LP_PERI_CLKRST.touch_ctrl.lp_touch_clk_en = enable;
}
...
{
    LP_PERI_CLKRST.touch_ctrl.lp_touch_rst_en = 1;
    LP_PERI_CLKRST.touch_ctrl.lp_touch_rst_en = 0;
}
...
{
    TOUCH_AON.scan_ctrl1.touch_xpd_wait = wait_cycles;
}
...
static inline void touch_ll_set_charge_times(uint8_t sample_cfg_id, uint16_t charge_times)
{
    ...
    case 0:
        TOUCH_AON.work_meas_num.touch_meas_num0 = charge_times;
        break;
    case 1:
        TOUCH_AON.work_meas_num.touch_meas_num1 = charge_times;
        break;
    case 2:
        TOUCH_AON.work_meas_num.touch_meas_num2 = charge_times;
        break;
    default:
        abort();
    }
...
static inline void touch_ll_get_charge_times(uint8_t sample_cfg_id, uint16_t *charge_times)
{
    switch (sample_cfg_id) {
    case 0:
        *charge_times = TOUCH_AON.work_meas_num.touch_meas_num0;
        break;
    case 1:
        *charge_times = TOUCH_AON.work_meas_num.touch_meas_num1;
        break;
    case 2:
        *charge_times = TOUCH_AON.work_meas_num.touch_meas_num2;
        break;
    default:
        abort();
    }
...
static inline void touch_ll_enable_fsm_timer(bool enable)
{
    ...
    TOUCH_AON.mux0.touch_fsm_en = enable;
    // Set 0 to stop by timer, otherwise by software
    TOUCH_AON.mux0.touch_done_force = !enable;
    // Set 0 to start by timer, otherwise by software
    TOUCH_AON.mux0.touch_start_force = !enable;
}
...
{
    return !TOUCH_AON.mux0.touch_start_force;
}
...
static inline void touch_ll_trigger_oneshot_measurement(void)
{
    ...
    TOUCH_AON.mux0.touch_start_en = 1;
    TOUCH_AON.mux0.touch_start_en = 0;
}

static inline void touch_ll_measure_channel_once(uint16_t chan_mask)
{
    TOUCH_AON.mux1.touch_start = chan_mask;
}
...
static inline void touch_ll_set_chan_active_threshold(uint32_t touch_num, uint8_t sample_cfg_id, uint32_t thresh)
{
    ...
    HAL_FORCE_MODIFY_U32_REG_FIELD(TOUCH_AON.padx_thn[touch_num].thresh[sample_cfg_id], threshold, thresh);  // codespell:ignore
}
...
{
    HAL_ASSERT(sample_cfg_id < SOC_TOUCH_SAMPLE_CFG_NUM);
    return HAL_FORCE_READ_U32_REG_FIELD(TOUCH_AON.padx_thn[touch_num].thresh[sample_cfg_id], threshold);  // codespell:ignore
}
...
static inline void touch_ll_enable_scan_mask(uint16_t chan_mask, bool enable)
{
    ...
    uint16_t prev_mask = TOUCH_AON.scan_ctrl1.touch_scan_pad_map;
    if (enable) {
        TOUCH_AON.scan_ctrl1.touch_scan_pad_map = prev_mask | mask;
    } else {
        TOUCH_AON.scan_ctrl1.touch_scan_pad_map = prev_mask & (~mask);
    }
}
...
static inline void touch_ll_enable_channel_mask(uint16_t enable_mask)
{
    ...
    TOUCH_AON.scan_ctrl1.touch_scan_pad_map = mask;
    TOUCH_AON.filter2.touch_outen = mask;
}
...
{
    TOUCH_AON.mux1.touch_xpd = chan_mask;
    TOUCH_AON.mux1.touch_start = chan_mask;
}
...
{
    uint32_t curr_mask = TOUCH_AON.mux1.touch_xpd;
    TOUCH_AON.mux1.touch_xpd = (~chan_mask) & curr_mask;
}
...
{
    TOUCH_AON.clr.touch_status_clr = 1;
}
...
static inline void touch_ll_read_chan_data(uint32_t touch_num, uint8_t sample_cfg_id, uint8_t type, uint32_t *data)
{
    ...
    TOUCH_AON.mux0.touch_freq_sel = sample_cfg_id;
    TOUCH_AON.mux0.touch_data_sel = type;
...
{
    TOUCH_AON.work.touch_out_sel = mode;
}
...
{
    TOUCH_AON.work.touch_out_gate = enable;
}
...
static inline void touch_ll_set_clock_div(uint8_t sample_cfg_id, uint32_t div_num)
{
    switch (sample_cfg_id) {
    case 0:
        TOUCH_AON.work.div_num0 = div_num - 1;
        break;
    case 1:
        TOUCH_AON.work.div_num1 = div_num - 1;
        break;
    case 2:
        TOUCH_AON.work.div_num2 = div_num - 1;
        break;
    default:
...
{
    if (timeout_cycles) {
        HAL_FORCE_MODIFY_U32_REG_FIELD(TOUCH_AON.scan_ctrl2, touch_timeout_num, timeout_cycles);
        TOUCH_AON.scan_ctrl2.touch_timeout_en = 1;
    } else {
        TOUCH_AON.scan_ctrl2.touch_timeout_en = 0;
    }
}
...
static inline void touch_ll_sample_cfg_set_engaged_num(uint8_t sample_cfg_num)
{
    ...
    TOUCH_AON.scan_ctrl2.freq_scan_en = !!sample_cfg_num;
    TOUCH_AON.scan_ctrl2.freq_scan_cnt_limit = sample_cfg_num ? sample_cfg_num : 1;
}
...
{
    uint32_t sample_cfg_num = TOUCH_AON.scan_ctrl2.freq_scan_cnt_limit;
...
{
    TOUCH_AON.scan_ctrl2.freq_scan_cnt_rise = rise_cnt;
}
...
{
    HAL_ASSERT(sample_cfg_id < SOC_TOUCH_SAMPLE_CFG_NUM);
    TOUCH_AON.freq_scan_para[sample_cfg_id].touch_freq_dcap_lpf = cap;
    TOUCH_AON.freq_scan_para[sample_cfg_id].touch_freq_dres_lpf = res;
}
...
{
    HAL_ASSERT(sample_cfg_id < SOC_TOUCH_SAMPLE_CFG_NUM);
    TOUCH_AON.freq_scan_para[sample_cfg_id].touch_freq_drv_ls = ls_drv;
    TOUCH_AON.freq_scan_para[sample_cfg_id].touch_freq_drv_hs = hs_drv;
}
...
{
    TOUCH_AON.clr.touch_channel_clr = chan_mask;
}
...
{
    TOUCH_AON.filter1.touch_filter_mode = mode;
}
...
{
    TOUCH_AON.filter1.touch_smooth_lvl = mode;
}
...
{
    TOUCH_AON.filter1.touch_debounce_limit = dbc_cnt;
}
...
static inline void touch_ll_filter_set_denoise_level(int denoise_lvl)
{
    ...
    TOUCH_AON.filter2.touch_bypass_noise_thres = always_update;
    TOUCH_AON.filter1.touch_noise_thres = always_update ? 0 : noise_thresh;

    TOUCH_AON.filter2.touch_bypass_nn_thres = always_update;
    TOUCH_AON.filter1.touch_nn_thres = always_update ? 0 : noise_thresh;
    TOUCH_AON.filter1.touch_nn_limit = 5; // 5 is the default value
}
...
{
    TOUCH_AON.filter1.touch_hysteresis = hysteresis;
}
...
{
    TOUCH_AON.filter1.touch_jitter_step = step;
}
...
{
    TOUCH_AON.filter1.touch_filter_en = enable;
}
 ...
 *
 * @param pad_num The pad number, range [0-13]
...
{
    TOUCH_AON.filter3.touch_update_benchmark_pad_sel = pad_num;
    TOUCH_AON.filter3.touch_update_benchmark_freq_sel = sample_cfg_id;
    HAL_FORCE_MODIFY_U32_REG_FIELD(TOUCH_AON.filter3, touch_benchmark_sw, benchmark);
    TOUCH_AON.filter3.touch_update_benchmark_sw = 1;
    // waiting for update
    while (TOUCH_AON.filter3.touch_update_benchmark_sw);
}
...
{
    TOUCH_AON.scan_ctrl2.touch_out_ring = pad_num;
}

/**
 * Enable or disable parameter of waterproof function.
 *
 * The waterproof function includes a shielded channel and a guard channel.
...
{
    TOUCH_AON.scan_ctrl1.touch_shield_pad_en = enable;
}
...
{
    TOUCH_AON.mux0.touch_bufsel = mask;
}
...
{
    TOUCH_AON.ana_para.touch_touch_buf_drv = driver_level;
}
...
static inline void touch_ll_set_proximity_sensing_channel(uint8_t prox_chan, uint32_t touch_num)
{
    switch (prox_chan) {
    case 0:
        TOUCH_AON.approach.touch_approach_pad0 = touch_num;
        break;
    case 1:
        TOUCH_AON.approach.touch_approach_pad1 = touch_num;
        break;
    case 2:
        TOUCH_AON.approach.touch_approach_pad2 = touch_num;
        break;
    default:
...
{
    HAL_FORCE_MODIFY_U32_REG_FIELD(TOUCH_AON.filter1, touch_approach_limit, scan_times);
}
...
static inline void touch_ll_proximity_set_charge_times(uint8_t sample_cfg_id, uint32_t charge_times)
{
    switch (sample_cfg_id) {
    case 0:
        TOUCH_AON.approach_work_meas_num.touch_approach_meas_num0 = charge_times;
        break;
    case 1:
        TOUCH_AON.approach_work_meas_num.touch_approach_meas_num1 = charge_times;
        break;
    case 2:
        TOUCH_AON.approach_work_meas_num.touch_approach_meas_num2 = charge_times;
        break;
    default:
        // invalid sample_cfg_id
        abort();
...
{
    if ((TOUCH_AON.approach.touch_approach_pad0 != touch_num)
            && (TOUCH_AON.approach.touch_approach_pad1 != touch_num)
            && (TOUCH_AON.approach.touch_approach_pad2 != touch_num)) {
        return false;
    }
...
{
    TOUCH_AON.slp0.touch_slp_pad = touch_num;
}
...
{
    *touch_num = (uint32_t)(TOUCH_AON.slp0.touch_slp_pad);
}
...
static inline void touch_ll_sleep_set_threshold(uint8_t sample_cfg_id, uint32_t touch_thresh)
{
    switch (sample_cfg_id) {
    case 0:
        HAL_FORCE_MODIFY_U32_REG_FIELD(TOUCH_AON.slp0, touch_slp_th0, touch_thresh);
        break;
    case 1:
        HAL_FORCE_MODIFY_U32_REG_FIELD(TOUCH_AON.slp1, touch_slp_th1, touch_thresh);
        break;
    case 2:
        HAL_FORCE_MODIFY_U32_REG_FIELD(TOUCH_AON.slp1, touch_slp_th2, touch_thresh);
        break;
    default:
        // invalid sample_cfg_id
        abort();
...
{
    TOUCH_AON.approach.touch_slp_approach_en = enable;
}
...
static inline void touch_ll_sleep_read_chan_data(uint8_t type, uint8_t sample_cfg_id, uint32_t *data)
{
    ...
    TOUCH_AON.mux0.touch_freq_sel = sample_cfg_id + 1;
    TOUCH_AON.mux0.touch_data_sel = type;
...
{
    TOUCH_AON.slp0.touch_slp_channel_clr = 1;
}
